const { createServer } = require("http"),
  ...
  html = String.raw,
  ramLimit = +`${readFileSync("/sys/fs/cgroup/memory.max")}`,
...
module.exports = (/**@type{Client<false>}*/ bot) => {
  ...
  bot.once("ready", async () => {
    ...
    createServer(({ url, headers }, res) => {
      ...
      const ramAvailable = () =>
          (
            ramLimit - `${readFileSync("/sys/fs/cgroup/memory.current")}`
...
alt><a
href=//discord.com/users/${owner.id}>${owner.tag.replace(/#0$/, "")}${
        owner.banner
          ? html`<img src=${owner.bannerURL({ size: 16 })}
alt>`
          : ""
      }<tr><th
title="RAM (bytes)">🆓🔀🧠<td><span
class=i
